    ...
    func readAnswerV3() {
        ...
        for i in 0..<columns {
            if rowResult[i] == "" {
                ...
                infText += "\(variable) has no leading 1.  This means it can be any real number.  Let \(variable) = \(parameters[i]).\n"
       ...
       // we have a leading one in the correct row
        for i in 0..<rows {
            ...
            if(matrix.grid[i][pivotColumn].num != 0 && i != pivotRow){
               ...
               // reciprocal.layer.borderWidth = 1
                reciprocal.sizeToFit()
                pivotRowImage.image = UIImage(named: "R\(pivotRow+1)")
                newRow.image = UIImage(named: "R\(i+1)")
//                operationHint.widthAnchor.constraint(equalTo: mainView.widthAnchor, multiplier: 0.95).isActive = true
//
        ...
        if(matrix.grid[pivotRow][pivotColumn] == Fraction(num: 1, den: 1)){
          ...
          //  let constant = Fraction(num: denominator, den: numerator)
           ...
           // hintText.text! += "Multiply Row \(pivotRow+1) by \(constant.num)/\(constant.den)."
            ...
            operationHint.widthAnchor.constraint(equalTo: mainView.widthAnchor, multiplier: 0.75).isActive = true
            //reciprocal.layer.borderWidth = 1
          //  reciprocal.sizeToFit()
